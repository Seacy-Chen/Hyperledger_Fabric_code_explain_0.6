...
// sendTransactionsToLocalEngine发送交易到本地engine(这个Peer是一个验证器)
// 下面p.engine.ProcessTransactionMsg，其中"p"代指PeerServer，engine是在创建PeerServer的时候
// 指定的Engine，而这个Engine的handler实现在Consensus里，在实现EngineHandler过程中加载了PBFT算法。
// 所以ProcessTransactionMsg函数的实现在consensus模块engine代码里
...
// Chat implementation of the the Chat bidi streaming RPC function
//handleChat执行过程中，建立消息循环，而这里的handler.HandleMessage。
// 这个handler是Engine的消息响应句柄，该消息响应处理来自于Consensus模块
