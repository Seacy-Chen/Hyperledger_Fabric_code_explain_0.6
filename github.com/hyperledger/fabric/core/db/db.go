...
var columnfamilies = []string{
	...
	stateCF,      // 世界状态
	stateDeltaCF, // 开启交易状态
	indexesCF,    // tx uuid -> 区块索引
	persistCF,    // 每一个块的持久状态 (共识)
}

// OpenchainDB 封装rocksdb的结构的结构体
...
// Create创建一个openchainDB实例
func create() *OpenchainDB {
	return &OpenchainDB{}
}

// GetDBHandle获取一个开放的openchainDB单列实例,注意 方法启动必须总是在这个方法之前调用
