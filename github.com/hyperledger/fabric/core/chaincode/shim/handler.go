...
// PeerChaincodeStream 处于Peer和chaincode实例之间的流接口
...
// NewChaincodeHandler返回一个shim层处理器新创建的实例
...
// 到目前为止还没有完，我们将invoke命令送给了Client的委托模块Shim进行处理。
// Shim模块根据来访事件类型送入指定处理函数
...
// handleMessage 链上代码或者验证器流的shim层消息处理循环。
func (handler *Handler) handleMessage(msg *pb.ChaincodeMessage) error {
	if msg.Type == pb.ChaincodeMessage_KEEPALIVE {
		// 接收到一个活的消息,什么都不做并且不触发状态机器
		return nil
	}
