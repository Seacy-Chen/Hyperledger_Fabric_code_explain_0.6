...
// NewDevopsServer 创建并且返回一个Devops服务实例
...
// 函数中d.coord是PeerServer对象，ExecuteTransaction是对应Engine的实现方法
func (d *Devops) invokeOrQuery(ctx context.Context, chaincodeInvocationSpec *pb.ChaincodeInvocationSpec, attributes []string, invoke bool) (*pb.Response, error) {
	...
	// d.coord 是PeerServer对象，ExecuteTransaction是对应Engine的实现方法
...
// EXP_GetApplicationTCert对已经提供的用户检验以个应用程序证书
...
// EXP_PrepareForTx 准备一个随后用到TX中的binding/TXHandler对。
...
// EXP_ProduceSigma产生一个sigma字符数组并且返回应答
...
// EXP_ExecuteWithBinding使用指定的binding/TXHandler来执行一个交易
