...
//处理创建镜像，负责将Client接入的信息传递到对应的Server模块
...
//_Devops_Invoke_Handler处理部署已经提供的链上代码镜像，其负责将Client接入的信息传递到对应的Server模块
...
// _Devops_Invoke_Handler函数负责将Client接入的信息传递到对应的Server模块
...
// DevopsServer设置Service规范，例如Invoke Message，调用_Devops_Invoke_Handler函数
