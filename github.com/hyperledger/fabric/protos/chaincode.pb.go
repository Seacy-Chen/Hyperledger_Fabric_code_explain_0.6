...
type ChaincodeID struct {
	...
	// 所有其他的请求将使用部署交易产生的这个名字(一个真正的hash编码)
...
// 在Register函数中，chainCodeSupport实例会 创建一个处理事件的handler，并运行processStream
...
//_ChaincodeSupport_Register_Handler函数，会调用chainCodeSupport实例的Register函数，
// 传入参数中有Server与client的通信媒介stream，在Register函数中，chainCodeSupport实例会
// 创建一个处理事件的handler，并运行processStream
...
//_ChaincodeSupport_serviceDesc 函数中Streams tag下存在Handler：_ChaincodeSupport_Register_Handler，
// 在chainCodeSupport实例注册完成后，会调用_ChaincodeSupport_Register_Handler函数，
// 该函数会调用chainCodeSupport实例的Register函数，传入参数中有Server与client
// 的通信媒介stream，在Register函数中，chainCodeSupport实例会创建一个处理事件的handler，
// 并运行processStream。
// 总结一下该流程：创建chainCodeSupport实例，并将该实例服务注册到grpcServer，
// grpcServer为该服务器生成stream，chainCodeSupport实例监控该stream的输入，
// 当有数据输入时调用handler的HandleMessage的数据处理，
// 经过分析判断chainCodeSupport的stream是与docker的chainCode通讯的媒介
